...
// The module's filename carries a content hash, so a rebuild is a new URL that
// no cache can answer from a stale copy. engine-version.js is generated by
// web/build.sh and is itself served with revalidation.
try {
  importScripts('engine-version.js');
} catch (e) {
  // Older deploy without the generated file: fall back to the fixed name.
}
const WASM_URL = self.ENGINE_WASM || 'dominion_wasm.wasm';
