...
The reason it matters beyond the search: the section above identified the
remaining ceiling as the quality of the search that generates the training
targets. That search has been running on the weaker of its two available leaf
estimates the entire time, which means every game in `selfplay-data/` carries
targets from a handicapped search.
...
- [ ] **Regenerate self-play with rollout leaves.** Every existing game was
      generated by a search running on the weaker leaf estimate, so the
      training targets are handicapped at the source.
- [ ] **A value head worth its cost.** The rollout wins on accuracy but costs
      4.2x. A value head trained to match rollout-quality targets would be
      both — that is the version worth having, and it needs the regenerated
      data first.
